  ...
  async (event) => {
    ...
    for (const key in metadata) {
      if (key.includes('generate_')) {
        const {filePrefix, height, width, webpVersion, folder} = unpackGenerateImageString(
          metadata[key]
        );
    ...
    await Promise.all(
      toGenerate.map(async (file) => {
        const buffer = await sharp(fileTemp)
          .resize(file.width || null, file.height || null, {
            fit: 'inside',
            background: 'white',
    ...
    if (webpToGenerate.length) {
      await Promise.all(
        webpToGenerate.map(async (file) => {
          const buffer = await sharp(file.source).webp({lossless: true}).toBuffer();
