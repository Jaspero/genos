<script>
  ...
  import { notification } from '../../config/store';
  ...
  <aside transition:fade={{ duration: 200 }}>
    ...
    <footer>
      {#if $notification.click}
        <button on:click={$notification.click}>{$notification.action || 'Set $notification.action'}</button>
      {/if}
...
<style>
  ...
  button + button {
    border-top: .1rem solid var(--fg-3);
  }
</style>
