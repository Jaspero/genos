  ...
  <div class="flex flex-wrap justify-start items-stretch gap-4">
    {#each filteredItems as item (item.id)}
      <div
        class="{item.used
          ? 'used-card'
          : 'coupon-card'} block rounded-lg bg-white w-[45%] xl:w-full shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] dark:bg-neutral-700"
      >
        <div class="flex flex-wrap h-full">
          <div class="flex flex-col flex-wrap justify-start flex-1 gap-4 p-6">
            <h5
              class="mb-2 text-xl font-medium leading-tight text-neutral-800 dark:text-neutral-50 text-center w-full"
            >
              {item.name}
            </h5>
          ...
          <div
            class="flex justify-center flex-col items-center gap-2 w-[30%] border-solid border-l-[1px] border-black"
          >
