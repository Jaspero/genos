...
function Navigation($$payload, $$props) {
  ...
  $$payload.out += `<header${attr("class", `navigation svelte-1kmwoty ${stringify([
    store_get($$store_subs ??= {}, "$page", page).url.pathname.includes("/posts/") ? "scrolled" : "",
    ""
  ].filter(Boolean).join(" "))}`)}><nav class="svelte-1kmwoty"><a${attr("href", store_get($$store_subs ??= {}, "$language", language) === "en" ? "/" : "/pocetna")} class="svelte-1kmwoty"><img class="logo svelte-1kmwoty" src="/brand/genos-logo-white.svg" alt="genos logo"></a> <div class="links svelte-1kmwoty"><!--[-->`;
  for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
    let link = each_array[$$index];
    $$payload.out += `<a${attr("href", store_get($$store_subs ??= {}, "$language", language) === "en" ? link.en.link : link.hr.link)}${attr("class", `svelte-1kmwoty ${stringify([
  ...
  $$payload.out += `<!--]--></div> <div class="flex gap-8"><button class="language-toggle svelte-1kmwoty"${attr("disabled", languageDisabled, true)}><span${attr("class", `svelte-1kmwoty ${stringify([
    store_get($$store_subs ??= {}, "$language", language) === "en" ? "active" : ""
  ].filter(Boolean).join(" "))}`)}>EN</span> <span${attr("class", `svelte-1kmwoty ${stringify([
    store_get($$store_subs ??= {}, "$language", language) === "hr" ? "active" : ""
  ].filter(Boolean).join(" "))}`)}>HR</span></button> <button class="menu-toggle svelte-1kmwoty"><img src="/icons/menu.svg" alt=""></button></div></nav></header> `;
  {
    $$payload.out += "<!--[!-->";
  }
  $$payload.out += `<!--]-->`;
