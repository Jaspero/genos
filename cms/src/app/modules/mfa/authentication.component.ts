...
export class AuthenticationComponent implements OnInit {
  ...
  submit() {
    this.subscription = from(this.afAuth.checkActionCode(this.oobCode))
      .pipe(
        switchMap(() => this.afAuth.applyActionCode(this.oobCode)),
        ...
        switchMap(session => {
          ...
          return phoneAuthProvider.verifyPhoneNumber(
            {phoneNumber: this.prefix + phone, session},
            this.recaptcha
          );
        }),
        catchError(e => {
  ...
  verify() {
    return () => {
      ...
      return from(this.mfa.enroll(multiFactorAssertion, 'Phone Number')).pipe(
        notify(),
        tap(() => {
