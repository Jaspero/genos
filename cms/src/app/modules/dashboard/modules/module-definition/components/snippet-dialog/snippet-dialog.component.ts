...
export class SnippetDialogComponent implements OnInit {
  ...
  ngOnInit() {
    this.snippetExamples$ = this.dbService
      .getExamples(ExampleType.Snippets)
      .pipe(
        queue(),
        map(res => res.data),
        shareReplay(1)
    ...
    this.snippetForm.get('snippet').valueChanges.subscribe(res => {
      this.data = undefined;
  ...
  submit() {
    this.snippetExamples$.pipe(take(1)).subscribe(examples => {
