...
export class DefinitionOverviewComponent implements OnInit {
  ...
  ngOnInit() {
    this.moduleExamples$ = this.dbService.getExamples(ExampleType.Modules).pipe(
      queue(),
      map(res => [
        {
          name: 'Empty',
  ...
  deleteOne(item: Module) {
    confirmation(
      [switchMap(() => this.dbService.removeModule(item.id)), notify()],
      {
  ...
  deleteSelection() {
    confirmation(
      ...
      {
        description: this.selection.selected.reduce(
          (acc, cur) => acc + cur + '\n',
  ...
  exportSelected(file = false) {
    this.items$.pipe(take(1)).subscribe(items => {
      this[file ? 'exportToFile' : 'export'](
        this.selection.selected.map(id => items.find(item => item.id === id))
      );
    });
  }
  ...
  export(schemes: any[]) {
    forkJoin(
      schemes.map(schema =>
        this.dbService.callFunction('cms-jsonSchemaToTypescript', schema)
      )
    )
      .pipe(
        queue(),
  ...
  selectModuleExample(item: Example) {
    ...
    this.router.navigate(['/module-definition/single/new'], {
      state: {example: item.json}
    });
  }

  openLayout() {
    this.dialog.open(LayoutSettingsComponent, {
      width: '700px'
    });
  }
}
