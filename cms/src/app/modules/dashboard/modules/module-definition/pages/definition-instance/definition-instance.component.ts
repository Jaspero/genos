...
import {
  Example,
  Snippet
} from '../../../../../../shared/interfaces/example.interface';
...
export class DefinitionInstanceComponent implements OnInit {
  ...
  ngOnInit() {
    this.roles$ = this.dbService
      .getDocumentsSimple(FirestoreCollection.Roles)
      .pipe(shareReplay(1));

    this.schemaValidation = new SchemaValidation();

    this.form$ = combineLatest([this.activatedRoute.params, this.import$]).pipe(
  ...
  openSnippetSelection(form) {
    this.dialog
      .open(SnippetDialogComponent, {
        width: '600px'
      })
      .afterClosed()
      .pipe(filter(res => res))
      .subscribe(([snippetFormValue, data, res]) => {
        const json = res.find(item => item.name === snippetFormValue.snippet)
          .json;
