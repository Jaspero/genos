...
export class InstanceOverviewContextService {
  ...
  loading$ = this.state.loadingQue$.pipe(map(items => !!items.length));
  routeData: any;
  ...
  openFilterDialog(data: FilterModule) {
    this.filterChange$
      .pipe(
        take(1),
      ...
      ],
      {
        description: this.selection.selected.reduce(
          ...
          `${this.transloco.translate(
            'INSTANCE_OVERVIEW.REMOVE_ITEMS_WARNING'
          )}\n`
        )
      }
    );
      ...
      [
        ...
        tap(() => {
          if (
            ...
            this.selection.selected.some(it => it === item.id)
          ) {
  ...
  export(columns?: ModuleLayoutTableColumn[]) {
    combineLatest([this.module$, this.filterChange$, this.sortChange$])
      .pipe(take(1))
