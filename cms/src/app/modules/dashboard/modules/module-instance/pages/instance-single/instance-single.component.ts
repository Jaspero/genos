...
export class InstanceSingleComponent implements OnInit {
          ...
          map((value: Partial<Module>) => {
            ...
            if (module.layout) {
              ...
              if (module.layout.instance) {
                ...
                ['formatOnSave', 'formatOnEdit', 'formatOnCreate'].forEach(
                  it => {
  ...
  save(instance: Instance) {
    return () => {
      ...
      const id =
        this.formBuilderComponent.form.getRawValue().id ||
        this.dbService.createId();

      return this.formBuilderComponent.save(instance.module.id, id).pipe(
        switchMap(() => {
