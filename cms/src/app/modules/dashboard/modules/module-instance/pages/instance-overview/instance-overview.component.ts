...
export class InstanceOverviewComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    this.ioc.module$.pipe(untilDestroyed(this)).subscribe(module => {
      const defaultData = {
        pageSize: null,
        sort: null,
        filter: null,
      ...
      this.ioc.pageSize = new FormControl(
        this.ioc.routeData.pageSize || DEFAULT_PAGE_SIZE
      );
      ...
      if (module.layout) {
        ...
        if (
          module.layout.filterModule &&
          module.layout.filterModule.value &&
        ...
        if (module.layout.overview) {
          ...
          if (module.layout.overview.toolbar) {
            this.toolbar = module.layout.overview.toolbar.reduce(
              (acc, cur: any) => {
                if (
                  typeof cur === 'string' ||
                  !cur.roles ||
                  cur.roles.includes(this.state.role)
                ) {
      ...
      this.ioc.items$ = combineLatest([
        this.ioc.pageSize.valueChanges.pipe(startWith(this.ioc.pageSize.value)),
        this.ioc.filterChange$,
        this.ioc.searchControl.valueChanges.pipe(
        ...
        switchMap(([pageSize, filter, search, sort]) => {
          ...
          return this.dbService
            .getDocuments(
              ...
              sort,
              null,
              this.generateFilters(module, search, filter)
            )
            .pipe(queue());
        }),
        switchMap(snapshots => {
          ...
          this.ioc.hasMore$.next(
            snapshots.length === this.ioc.routeData.pageSize
          );
  ...
  getCurrentView(selector: string) {
    ...
    const toUse = selector.startsWith(STATIC_CONFIG.elementSelectorPrefix)
      ? selector
      : STATIC_CONFIG.elementSelectorPrefix + selector;
      ...
      ? [
          {
            ...
            operator: module.layout.searchModule.simple
              ? FilterMethod.Equal
              : FilterMethod.ArrayContains,
