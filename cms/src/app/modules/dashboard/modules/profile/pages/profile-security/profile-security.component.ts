...
export class ProfileSecurityComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.multipleProviders$ = this.afAuth.user.pipe(
      map(item => item?.providerData?.length > 1)
    );

    this.hasTwoFactor$ = this.afAuth.user.pipe(
      map(user => !!user.multiFactor?.enrolledFactors?.length)
    );

    this.googleProvider$ = this.afAuth.user.pipe(
      map(item =>
        (item?.providerData || []).find(it => it.providerId === 'google.com')
      )
    );

    this.passwordProvider$ = this.afAuth.user.pipe(
      map(item =>
        (item.providerData || []).find(it => it.providerId === 'password')
      )
    );
  ...
  changePassword(form: FormGroupDirective) {
    return () =>
      from(
        firebase
          .auth()
          .currentUser.updatePassword(this.pwForm.get('password').value)
      ).pipe(
        catchError(err => {
          let message;
          ...
          if (err.code === 'auth/requires-recent-login') {
            message =
              'For security reasons please login to your account again before changing your password.';
            firebase
              .auth()
              .signOut()
  ...
  changeEmail() {
    return () => {
      ...
      return combineLatest([
        ...
        from(firebase.auth().currentUser.updateEmail(email)).pipe(
          catchError(err => {
            let message;

            if (err.code === 'auth/requires-recent-login') {
              message =
                'For security reasons please login to your account again before changing your email.';
              firebase
                .auth()
                .signOut()
                .then(() => this.router.navigate(['/login']));
  ...
  removeAccount() {
    confirmation(
      [
        ...
        notify({
          success:
            'Your account and all of your personal information have been removed from our system.'
        }),
        tap(() => {
          this.router.navigate(['/login']);
        })
  ...
  removeProvider(provider: string) {
    confirmation(
      [
        switchMap(() => firebase.auth().currentUser.unlink(provider)),
        notify({
              ...
              from(
                hasTwoFactor
                  ? user.multiFactor.unenroll(
                      user.multiFactor.enrolledFactors.pop()
                    )
