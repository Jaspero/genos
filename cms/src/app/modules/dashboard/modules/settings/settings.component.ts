...
export class SettingsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.languageControl = new FormControl(this.state.language, [
      Validators.required
    ]);
    ...
    this.dbService
      ...
      .subscribe(([roles, ...users]: any) => {
        users = users.map(it =>
          it[0] ? {...it[0].data(), id: it[0].id} : null
        );
  ...
  remove(user: User) {
    return () => {
      this.users.splice(
        this.users.findIndex(it => it.email === user.email),
        1
      );
