...
export class LayoutComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.links$ = combineLatest([this.state.layout$, this.state.modules$]).pipe(
      map(([layout, modules]) => {
        if (layout.navigation) {
          return layout.navigation.items.reduce((acc, item) => {
            if (!item.authorized || item.authorized.includes(this.state.role)) {
              if (item.function) {
              ...
              acc.push({
                ...
                ...(item.children
                  ? {
                      children: item.children.reduce((a, c) => {
                        if (
                          !c.authorized ||
                          c.authorized.includes(this.state.role)
                        ) {
                          if (c.function) {
          ...
          const links: NavigationItemWithActive[] = modules.reduce(
            (acc, item) => {
              if (
                !item.authorization ||
