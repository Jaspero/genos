...
export function swiperContainer(domComponents: any, config: any = {}) {
	...
	domComponents.addType(containerName, {
				...
				script() {
					...
					async function render() {
						...
						if (window.location !== window.parent.location) {
							...
							window.swipers['{[ swiperId ]}'] = {
								swiper: (el: HTMLDivElement) => new Swiper(el, options),
								id
							};
						}
					}
		...
		isComponent: (el: any) => el.hasAttribute && el.hasAttribute(containerId),
		view: {
			init() {
				const props = [
					'initialSlide',
