...
<script lang="ts">
  import { collection, getDocs, orderBy, query, limit, where, QueryConstraint, QueryDocumentSnapshot, startAt } from 'firebase/firestore';
  ...
  async function typeChanges(pageSize: string, category?: string, author?: string, authorMap?: MapItem) {
	...
	async function loadMore() {
		...
		articles = [
			...articles,
			...docs.map((doc) => doc.data() as BlogArticleSnippet)
		];
	}
