...
export class FbDatabaseService extends DbService {
  ...
  removeModule(id: string) {
    return from(
      this.afs.collection(FirestoreCollection.Modules).doc(id).delete()
    );
  }
  ...
  getDocuments(moduleId, pageSize, sort?, cursor?, filters?, source?) {
  ...
  removeDocument(moduleId, documentId) {
    return from(this.afs.collection(moduleId).doc(documentId).delete());
  }
